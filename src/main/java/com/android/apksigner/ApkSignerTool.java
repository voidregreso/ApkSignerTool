...
public class ApkSignerTool {
    ...
    public static String verify(String[] params) throws Exception {
        ...
        while ((optionName = optionsParser.nextOption()) != null) {
            ...
            switch (optionName) {
                ...
                case "print-certs" -> printCerts = optionsParser.getOptionalBooleanValue(true);
                case "print-certs-pem" -> {
                ...
                case "Werr" -> warningsTreatedAsErrors = optionsParser.getOptionalBooleanValue(true);
                case "v4-signature-file" -> v4SignatureFile = new File(optionsParser.getRequiredValue(
                        "Input V4 Signature File"));
                case "in" -> inputApk = new File(optionsParser.getRequiredValue("Input APK file"));
                case "verify-source-stamp" -> verifySourceStamp = optionsParser.getOptionalBooleanValue(true);
                case "stamp-cert-digest" -> sourceCertDigest = optionsParser.getRequiredValue(
        ...
        if (verified) {
            ...
            if (verbose) {
                output.append("Verified using v1 scheme (JAR signing): ").append(result.isVerifiedUsingV1Scheme()).append("\n");
                output.append("Verified using v2 scheme (APK Signature Scheme v2): ").append(result.isVerifiedUsingV2Scheme()).append("\n");
                output.append("Verified using v3 scheme (APK Signature Scheme v3): ").append(result.isVerifiedUsingV3Scheme()).append("\n");
                output.append("Verified using v3.1 scheme (APK Signature Scheme v3.1): ").append(result.isVerifiedUsingV31Scheme()).append("\n");
                output.append("Verified using v4 scheme (APK Signature Scheme v4): ").append(result.isVerifiedUsingV4Scheme()).append("\n");
                output.append("Verified for SourceStamp: ").append(result.isSourceStampVerified()).append("\n");
                if (!verifySourceStamp) {
                    output.append("Number of signers: ").append(signerCerts.size()).append("\n");
                }
            }
            if (printCerts) {
    ...
    public static void printCertificate(X509Certificate cert, String name, boolean verbose,
        ...
        if (pemOutput) {
            ...
            for (int i = 0; i < pemEncodedCert.length(); i += lineWidth) {
                System.out.println(pemEncodedCert.substring(i, Math.min(i + lineWidth, pemEncodedCert.length())));
            }
